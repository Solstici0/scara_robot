...
class TrayectoryParams:
    """
    Class to store the parameters used by the trayectory method in order to
    generate a curve
    """
    def __init__(self,
                 freq,
...
class Trayectory():
    """
    Class to store the output of a trayectory generation for a single axis
    each parameter should be an array containing the said curve, except for
    params wich is a TrayectoryParams object used to generate the curve
    """
    def __init__(self,result, params: TrayectoryParams = None) -> None:
        self.position = result[0]
        self.speed = result[1]
        self.acceleration = result[2]
        self.jerk = result[3]
        self.params = params
    def plot(self):
        totalTime=np.linspace(1.0/500.0,len(self.jerk)/500.0,len(self.jerk))
        plt.subplot(4,1,1)
        plt.plot(totalTime,self.jerk)
        plt.ylabel(r'$\frac{rads}{s^e}$',rotation=0)
        plt.grid()
        plt.subplot(4,1,2)
        plt.plot(totalTime,self.acceleration)
        plt.ylabel(r'$\frac{rads}{s^2}$',rotation=0 )
        plt.grid()
        plt.subplot(4,1,3)
        plt.plot(totalTime,self.speed)
        plt.ylabel(r'$\frac{rads}{s}$',rotation=0 )
        plt.grid()
        plt.subplot(4,1,4)
        plt.plot(totalTime,self.position)
        plt.ylabel(r'$rads$',rotation=0 )
        plt.xlabel('tiempo [s]')
        plt.grid()
        plt.show()

class Scara_trayectory():
    """
    Class to store all of the trayectories of an XY movement in an scara,
    each parameter is a Trayectory object, except for params, hombro and codo
    have the inverse kinematics of x_trayectory and Y_trayectory
    """
    def __init__(self, params:TrayectoryParams,
                 axis0:list,
                 axis1:list,
                 config:str) -> None:
        self.params = params
        ...
        if config == 'xy':
            #movement generated from an xy trayectory
            delta = self.params.freq
            self.x = Trayectory(axis0,params)
            self.y = Trayectory(axis1,params)
            length = len(self.x.position)
            hombro_data = [[0.0]*length,[0.0]*length,[0.0]*length,[0.0]*length]
            codo_data =[[0.0]*length,[0.0]*length,[0.0]*length,[0.0]*length]
            for i in range(length):
                this_point = kin.inverse_kin(self.x.position[i],self.y.position[i])
                hombro_data[0][i] = this_point['hombro']
                codo_data[0][i] = this_point['codo']
                if i>0:
                    custom_differential(hombro_data,i,delta)
                    custom_differential(codo_data,i,delta)

            self.hombro = Trayectory(hombro_data,params)
            self.codo = Trayectory(codo_data,params)
        else:
            raise Exception('not implemented')

def custom_differential(input_array,i,delta):
    input_array[1][i] = (input_array[0][i]-input_array[0][i-1])*delta
    input_array[2][i] = (input_array[1][i]-input_array[1][i-1])*delta
    input_array[3][i] = (input_array[2][i]-input_array[2][i-1])*delta
...
def trayectoria(distance,frec=500.0, A_max=15.0, V_max=9.9484, J_max=450.0, S_max=400.0):
    ...
    if Ts==Td_s:
        ...
        timeV=np.linspace(1.0/frec,Tv,int(Tv*frec))
        ...
        if Tv_a>=Td_a:
            ...
            timeA=np.linspace(1.0/frec,Ta,int(Ta*frec))
        ...
        if Tj==Td_j:
            ...
            timeJ=np.linspace(1.0/frec,Tj,int(Tj*frec))
...
def s_curve(distance:float, params:TrayectoryParams)->Trayectory:
    result = Trayectory(trayectoria(distance,
                         frec=params.freq,
                         A_max=params.acceleration_limit,
                         V_max=params.speed_limit,
                         J_max=params.jerk_limit,
                         S_max=params.s_variable),
                         params)
    return result


def two_joints(d_hombro,d_codo,S=50.0,frec=500.,config_dict =None):
    ...
    if config_dict:
        A_hombro = config_dict['ACC_J0']
        V_hombro = config_dict['VEL_J0']
        J_hombro = config_dict['JERK_J0']
        S_hombro = S
        A_codo = config_dict['ACC_J1']
        V_codo = config_dict['VEL_J1']
        J_codo = config_dict['JERK_J1']
        S_codo = S
...
def dual_axis_euclidean(point0,point1,params):
    """
    Generates a joint trayectory asumming a linear movement in the combined axys
    """
    axis_delta = [point1[0]-point0[0], point1[1]-point0[1]]
    delta = np.sqrt((point1[0]-point0[0])**2+(point1[1]-point0[1])**2)
    single_tray = s_curve(delta,params) 
    axis0 = [[i*axis_delta[0]/delta+point0[0] for i in single_tray.position],
             [i*axis_delta[0]/delta for i in single_tray.speed],
             [i*axis_delta[0]/delta for i in single_tray.acceleration],
             [i*axis_delta[0]/delta for i in single_tray.jerk]]
    axis1 = [[i*axis_delta[1]/delta+point0[1] for i in single_tray.position],
             [i*axis_delta[1]/delta for i in single_tray.speed],
             [i*axis_delta[1]/delta for i in single_tray.acceleration],
             [i*axis_delta[1]/delta for i in single_tray.jerk]]
    output_tray = Scara_trayectory(params,axis0,axis1,'xy')
    return output_tray

if __name__=='__main__':
    # curves = trayectoria(distance = 100.0)
    # curvalarga()
    # Start an IPython terminal
    params = TrayectoryParams(freq=500,
                              speed_limit=1000,
                              acceleration_limit=100,
                              jerk_limit=20,
                              s_variable=5)
    scara_curve = dual_axis_euclidean([-100,400],[100,500],params)
